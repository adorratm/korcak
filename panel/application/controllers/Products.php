...
class Products extends MY_Controller
{
    ...
    public function new_form()
    {
        ...
        $viewData->technical_informations = $this->general_model->get_all("technical_informations", null, null, ["isActive" => 1, "lang" => $this->session->userdata('activeLang')]);
    ...
    {
        $items = $this->product_dimension_model->getRows(["product_id" => $product_id, "lang" => $this->session->userdata('activeLang')], $_POST);
        $data = [];
        $i = (!empty($_POST['start']) ? $_POST['start'] : 0);
        if (!empty($items)) :
            foreach ($items as $item) :
