...
class Service_categories extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $viewData->items = $this->service_category_model->get_all(["lang" => $this->session->userdata('activeLang')]);
        $this->load->view("{$viewData->viewFolder}/{$viewData->subViewFolder}/index", $viewData);
    }
    public function datatable()
    {
        $items = $this->service_category_model->getRows(["lang" => $this->session->userdata('activeLang')], $_POST);
        $data = [];
        ...
        $output = [
            ...
            "recordsTotal" => $this->service_category_model->rowCount(["lang" => $this->session->userdata('activeLang')]),
            "recordsFiltered" => $this->service_category_model->countFiltered(["lang" => $this->session->userdata('activeLang')], (!empty($_POST) ? $_POST : [])),
            "data" => $data,
        ];
    ...
    public function delete($id)
    {
        ...
        if (!empty($service_category)) :
            ...
            if ($delete) :
                 ...
                 */
                $services = $this->service_model->get_all(["lang" => $this->session->userdata('activeLang')]);
                if (!empty($services)) :
                    foreach ($services as $key => $value) :
                        if ($value->category_id == $id) :
                            $this->service_model->delete(["id" => $value->id]);
                            if (!empty($value->img_url)) :
                                if (!is_dir(FCPATH . "uploads/services_v/{$value->img_url}") && file_exists(FCPATH . "uploads/services_v/{$value->img_url}")) :
                                    unlink(FCPATH . "uploads/services_v/{$value->img_url}");
                                endif;
                            endif;
                        endif;
                    endforeach;
