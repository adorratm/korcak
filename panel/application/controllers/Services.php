...
class Services extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $viewData->items = $this->service_model->get_all(["lang" => $this->session->userdata('activeLang')], "rank ASC");
        $this->load->view("{$this->viewFolder}/{$viewData->subViewFolder}/index", $viewData);
    }
    public function datatable()
    {
        $items = $this->service_model->getRows(["lang" => $this->session->userdata('activeLang')], $_POST);
        $data = [];
            ...
            foreach ($items as $item) :
                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu rounded-0 dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                        ...
                        <a class="dropdown-item remove-btn" href="javascript:void(0)" data-table="servicesTable" data-url="' . base_url("services/delete/$item->id") . '"><i class="fa fa-trash mr-2"></i>Kaydı Sil</a>
                    </div>
                </div>';
        ...
        $output = [
            ...
            "recordsTotal" => $this->service_model->rowCount(["lang" => $this->session->userdata('activeLang')]),
            "recordsFiltered" => $this->service_model->countFiltered(["lang" => $this->session->userdata('activeLang')], (!empty($_POST) ? $_POST : [])),
            "data" => $data,
        ];
    ...
    public function save()
    {
        ...
        if (checkEmpty($data)["error"]) :
            ...
            $getRank = $this->service_model->rowCount(["lang" => $this->session->userdata('activeLang')]);
            if (!empty($_FILES)) :
                ...
                if (!empty($_FILES["banner_url"]["name"])) :
                    ...
                    if ($image["success"]) :
                        ...
                        echo json_encode(["success" => false, "title" => "Başarısız!", "message" => "Hizmet Kaydı Yapılırken Hata Oluştu. Hizmet Banner Görseli Seçtiğinizden Emin Olup Tekrar Deneyin."]);
                        die();
                    endif;
                endif;
            endif;
    ...
    public function update($id)
    {
        ...
        if (checkEmpty($data)["error"]) :
            ...
            $data["banner_url"] = $service->banner_url;
            if (!empty($_FILES["img_url"]["name"])) :
                $image = upload_picture("img_url", "uploads/$this->viewFolder",[],"*");
                if ($image["success"]) :
                    $data["img_url"] = $image["file_name"];
                    if (!empty($service->img_url)) :
                        if (!is_dir(FCPATH . "uploads/{$this->viewFolder}/{$service->img_url}") && file_exists(FCPATH . "uploads/{$this->viewFolder}/{$service->img_url}")) :
                            unlink(FCPATH . "uploads/{$this->viewFolder}/{$service->img_url}");
                        endif;
                    endif;
                else :
                    echo json_encode(["success" => false, "title" => "Başarısız!", "message" => "Hizmet Güncelleştirilirken Hata Oluştu. Hizmet Görseli Seçtiğinizden Emin Olup Tekrar Deneyin."]);
                    die();
                endif;
            endif;
            if (!empty($_FILES["banner_url"]["name"])) :
                ...
                if ($image["success"]) :
                    $data["banner_url"] = $image["file_name"];
                    if (!empty($service->banner_url)) :
                        if (!is_dir(FCPATH . "uploads/{$this->viewFolder}/{$service->banner_url}") && file_exists(FCPATH . "uploads/{$this->viewFolder}/{$service->banner_url}")) :
                            unlink(FCPATH . "uploads/{$this->viewFolder}/{$service->banner_url}");
                        endif;
                    endif;
                else :
                    echo json_encode(["success" => false, "title" => "Başarısız!", "message" => "Hizmet Güncelleştirilirken Hata Oluştu. Hizmet Banner Görseli Seçtiğinizden Emin Olup Tekrar Deneyin."]);
                    die();
                endif;
            endif;
            $data["seo_url"] = seo($data["title"]);
            $data["content"] = $_POST["content"];
            $update = $this->service_model->update(["id" => $id], $data);
            if ($update) :
    ...
    public function rankSetter()
    {
        ...
        if (!empty($rows)) :
            foreach ($rows as $row) :
                $this->service_model->update(["id" => $row["id"]], ["rank" => $row["position"]]);
            endforeach;
        endif;
    }
