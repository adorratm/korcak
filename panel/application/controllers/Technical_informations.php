...
class Technical_informations extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $items = $this->technical_information_model->get_all(["lang" => $this->session->userdata('activeLang')], "rank ASC");
    ...
    {
        $items = $this->technical_information_model->getRows(["lang" => $this->session->userdata('activeLang')], $_POST);
        $data = [];
        ...
        $output = [
            ...
            "recordsTotal" => $this->technical_information_model->rowCount(["lang" => $this->session->userdata('activeLang')]),
            "recordsFiltered" => $this->technical_information_model->countFiltered(["lang" => $this->session->userdata('activeLang')], (!empty($_POST) ? $_POST : [])),
            "data" => $data,
        ];
    ...
    public function new_form()
    {
        ...
        $viewData->categories = $this->technical_information_category_model->get_all(["lang" => $this->session->userdata('activeLang')]);
    ...
    public function save()
    {
        ...
        if (checkEmpty($data)["error"] && checkEmpty($data)["key"] !== "content") :
            ...
            $getRank = $this->technical_information_model->rowCount(["lang" => $this->session->userdata('activeLang')]);
    ...
    public function delete($id)
    {
        ...
        if (!empty($technical_information)) :
            $technical_information_images = $this->technical_information_image_model->get_all(["technical_information_id" => $id,"lang" => $this->session->userdata('activeLang')]);
    ...
    public function deleteBulk()
    {
        ...
        $technical_informations = $this->general_model->get_all("technical_informations", null, null, ["lang" => $this->session->userdata('activeLang')], [], [], [], ["id" => $technical_informationIds]);
        if (!empty($technical_informations)) :
            ...
            if ($delete) :
                ...
                $technical_information_images = $this->general_model->get_all("technical_information_images", null, null, ["lang" => $this->session->userdata('activeLang')], [], [], [], ["technical_information_id" => $technical_informationIds]);
                if (!empty($technical_information_images)) :
                    foreach ($technical_information_images as $key => $value) :
                        if (!is_dir(FCPATH . "uploads/{$this->viewFolder}/{$value->url}") && file_exists(FCPATH . "uploads/{$this->viewFolder}/{$value->url}")) :
                            unlink(FCPATH . "uploads/{$this->viewFolder}/{$value->url}");
    ...
    public function detailDatatable($technical_information_id)
    {
        $items = $this->technical_information_image_model->getRows(
            ["technical_information_id" => $technical_information_id, "lang" => $this->session->userdata('activeLang')],
            $_POST
        );
        $data = [];
        $i = (!empty($_POST['start']) ? $_POST['start'] : 0);
        ...
        $output = [
            "draw" => (!empty($_POST['draw']) ? $_POST['draw'] : 0),
            "recordsTotal" => $this->technical_information_image_model->rowCount(["technical_information_id" => $technical_information_id, "lang" => $this->session->userdata('activeLang')]),
            "recordsFiltered" => $this->technical_information_image_model->countFiltered(["technical_information_id" => $technical_information_id, "lang" => $this->session->userdata('activeLang')], (!empty($_POST) ? $_POST : [])),
            "data" => $data,
        ];
        // Output to JSON format
        echo json_encode($output);
    ...
    public function upload_form($id)
    {
        ...
        $viewData->item = $this->technical_information_model->get(["id" => $id]);
        $viewData->settings = $this->general_model->get_all("settings", null, null, ["isActive" => 1]);
        $viewData->items = $this->technical_information_image_model->get_all(["technical_information_id" => $id, "lang" => $this->session->userdata('activeLang')], "rank ASC");
        $this->load->view("{$viewData->viewFolder}/{$viewData->subViewFolder}/index", $viewData);
    }
    ...
    public function file_upload($technical_information_id, $lang)
    {
        ...
        if ($image["success"]) :
            $getRank = $this->technical_information_image_model->rowCount(["lang" => $this->session->userdata('activeLang')]);
    ...
    {
        $items = $this->technical_information_dimension_model->getRows(["technical_information_id" => $technical_information_id, "lang" => $this->session->userdata('activeLang')], $_POST);
        $data = [];
        $i = (!empty($_POST['start']) ? $_POST['start'] : 0);
        if (!empty($items)) :
            foreach ($items as $item) :
        ...
        $output = [
            "draw" => (!empty($_POST['draw']) ? $_POST['draw'] : 0),
            "recordsTotal" => $this->technical_information_dimension_model->rowCount(["technical_information_id" => $technical_information_id, "lang" => $this->session->userdata('activeLang')]),
            "recordsFiltered" => $this->technical_information_dimension_model->countFiltered(["technical_information_id" => $technical_information_id, "lang" => $this->session->userdata('activeLang')], (!empty($_POST) ? $_POST : [])),
            "data" => $data,
        ];
        // Output to JSON format
        echo json_encode($output);
    ...
    public function dimensionsSave()
    {
        ...
        $getRank = $this->technical_information_dimension_model->rowCount(["lang" => $this->session->userdata('activeLang')]);
        $data["title"] = @stripslashes($data["title"]);
        $data["isActive"] = 1;
        $data["rank"] = $getRank + 1;
